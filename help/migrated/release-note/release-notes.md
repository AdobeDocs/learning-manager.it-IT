...
**Widget salvato dall&#39;utente**

* **Amministratore:** gli Amministratori possono inserire il widget Salvato da me in una pagina in modo che gli Allievi possano trovare facilmente i contenuti contrassegnati con un segnalibro. Ciò consente agli Allievi di tornare ai corsi salvati in precedenza senza doverli cercare o accedere nuovamente. Ulteriori informazioni sul widget [Salvato da me](/help/migrated/administrators/feature-summary/experience-builder/add-a-widget.md#bookmark-widget).

* **Allievo:** il widget **Salvato da me** mostra corsi, percorsi di apprendimento, certificazioni e risorse formative con segnalibro per cui hai effettuato l’accesso in seguito. Utilizzate questo widget per accedere rapidamente ai contenuti salvati in un’unica posizione, senza eseguire nuovamente la ricerca nel catalogo. Ulteriori informazioni sul widget [Salvato da me](/help/migrated/learners/feature-summary/saved-by-me-widget.md).

**Colore tema canale:** Quando crei un canale, hai più colori tema canale tra cui scegliere. I colori del tema vengono applicati contestualmente per evidenziare il contrasto dopo la selezione. Quando selezionate un colore tema per il canale, il colore selezionato viene applicato all’interfaccia del canale ed è progettato per mantenere il contrasto con le miniature dei canali e il testo. Un colore predefinito viene assegnato automaticamente quando create un canale, ma potete scegliere in qualsiasi momento un colore diverso dalle opzioni disponibili.

**Miniature video:** l’Allievo ora è in grado di identificare rapidamente un video in base al riquadro, che a sua volta visualizza una miniatura per aiutarlo a identificarne il contenuto a colpo d’occhio. Se il video sorgente non dispone di una propria miniatura, Adobe Learning Manager ne genera automaticamente una usando il colore del tema del canale, un pattern astratto e il titolo del video.

**Canali:** l’opzione di navigazione Canali è visibile agli allievi solo quando è stato creato almeno un canale, contiene almeno un video dalla sua origine ed è contrassegnata come Visibile agli allievi. Ulteriori informazioni su [Canali](/help/migrated/administrators/feature-summary/create-channels.md).

## Bug corretti

**Componente Adobe Experience Manager:** dopo l&#39;aggiornamento al pacchetto del sito di riferimento di Adobe Learning Manager v2.0.1, il pannello delle notifiche non è stato caricato quando gli utenti hanno fatto clic sulla campana delle notifiche. Il caricatore continuava a girare all&#39;infinito, impedendo la visualizzazione delle notifiche. Il problema ha causato anche chiamate ripetute all&#39;endpoint /userNotifications, con conseguenti risposte HTTP 429. Il flusso di lavoro di caricamento delle notifiche è stato corretto per evitare richieste API eccessive e risolvere il problema di caricamento. Le notifiche ora vengono caricate correttamente e il pannello si apre come previsto senza limitazioni API non necessarie.

**App per dispositivi mobili Adobe Learning Manager > Percorso di apprendimento:** i corsi completati all&#39;interno di un percorso di apprendimento nell&#39;app per dispositivi mobili Adobe Learning Manager sono stati visualizzati in modo errato con un indicatore giallo e un punto rosso, anche se il loro stato era contrassegnato come completato. Questa incoerenza visiva si è verificata solo per i corsi all’interno dei percorsi di apprendimento, mentre i moduli completati venivano visualizzati correttamente. La logica dell’indicatore di stato è stata corretta per i corsi del percorso di apprendimento nell’app per dispositivi mobili. I corsi completati ora visualizzano l’indicatore verde di completamento corretto. Ciò garantisce un&#39;esperienza di completamento uniforme nell&#39;app per dispositivi mobili e nel browser per desktop.

+++


+++Aggiornamento 110: versione del 16 agosto 2026 di Adobe Learning Manager

## Rilascia evidenziazioni

La versione di agosto 2026 di Adobe Learning Manager offre progressi significativi per l&#39;apprendimento personalizzato, l&#39;intelligenza artificiale, la creazione di report e le integrazioni.
...
**Trascrizione Allievo:** Le date nel report Trascrizione Allievo registravano 00 per secondi, mentre nel webhook i secondi venivano visualizzati come dovrebbero, in due cifre. Questi problemi sono stati risolti. La Trascrizione Allievo ora mostra correttamente anche i secondi a due cifre.
...
### L’Istruttore non può unirsi all’Aula virtuale tra i moduli

Gli istruttori non possono entrare nell’aula virtuale quando tentano di entrare in una stanza al di fuori dell’orario pianificato del modulo del corso assegnato.

Adobe Learning Manager utilizza un’unica aula virtuale per tutti i moduli di un corso. Quando un Istruttore avvia la stanza, la sessione è associata al modulo collegato alla partecipazione di quell’Istruttore
URL. Qualsiasi richiesta di join successiva deve corrispondere al modulo attivo associato alla room. Se un altro istruttore tenta di partecipare utilizzando un collegamento di join di un altro modulo mentre è attivo un altro modulo, accedi al
la stanza è negata. Nella sala condivisa può essere attivo un solo modulo alla volta.

Questo comportamento è previsto nella versione corrente. I miglioramenti per consentire agli Istruttori del corso di entrare nella stanza indipendentemente dal modulo attivo sono pianificati per una versione futura.

### L’audio si interrompe quando il microfono e l’altoparlante differiscono su macOS

L’audio può interrompersi in modo intermittente o il suono si inceppa durante una sessione aula virtuale su macOS quando il microfono e l’altoparlante selezionati sono dispositivi diversi.

Questo comportamento può verificarsi quando vengono utilizzati dispositivi diversi per l’ingresso e l’uscita audio, ad esempio AirPods per il microfono e l’altoparlante incorporato per la riproduzione. Poiché ogni dispositivo introduce un proprio ritardo audio, l&#39;eco
la cancellazione è meno efficace e la soppressione del rumore di fondo può occasionalmente identificare parti del discorso come rumore. Ciò può causare brevi interruzioni audio. Il problema è più evidente quando
la voce dell&#39;altoparlante viene catturata a un volume basso, ad esempio quando il microfono è posizionato più lontano.

Si tratta di una limitazione nota della piattaforma e non è specifica di Adobe Learning Manager. Un comportamento simile è stato osservato in altre applicazioni per conferenze. Questa versione non include alcuna correzione e il problema è in fase di valutazione per una versione futura.

Utilizzare lo stesso dispositivo sia per il microfono che per gli altoparlanti per evitare questo comportamento.

### Il video condiviso si blocca in Chrome dopo aver perso lo stato attivo

Quando un Istruttore condivide una finestra di Chrome riproducendo un video e quindi sposta lo stato attivo su di esso, i partecipanti potrebbero vedere il contenuto condiviso congelarsi o visualizzare una schermata nera.
...
Si tratta di una limitazione del browser e del sistema operativo e non è specifica di Adobe Learning Manager. Non è disponibile alcuna correzione specifica per il prodotto per questa versione.

Durante la presentazione dei contenuti video, la finestra del browser condiviso resta a fuoco. Evita di tornare alla finestra Aula virtuale durante la riproduzione.

### L’Istruttore non può partecipare a un modulo già in uso

Quando gli Istruttori tentano di avviare due moduli di corso che utilizzano contemporaneamente la stessa aula virtuale, solo il primo Istruttore può entrare correttamente nella stanza.

Adobe Learning Manager utilizza una singola classe virtuale condivisa per un’istanza del corso. Quando il primo Istruttore entra nella stanza, questa viene associata al modulo dell’Istruttore. Le richieste di partecipazione aggiuntive sono
convalidato rispetto al modulo attivo. Se un altro Istruttore tenta di partecipare utilizzando un collegamento di modulo diverso, l’accesso viene negato. Di conseguenza, le sessioni simultanee tenute da un Istruttore non possono essere condotte all’interno dello stesso
istanza del corso utilizzando moduli separati che condividono una room.

Questo comportamento è previsto nella versione corrente ed è coerente con l’architettura della sala esistente. La funzionalità è in fase di revisione per essere migliorata in futuro in base al feedback dei clienti.

### Il contenuto condiviso visualizza un effetto di tunneling dopo il ritorno alla finestra Hub dinamico

Quando un Istruttore condivide l’intero schermo e poi torna alla finestra Hub dal vivo, i partecipanti potrebbero riscontrare un effetto di tunneling nel contenuto condiviso. Quando condividi l’intero schermo, il feed condiviso include tutti gli elementi visualizzati sul desktop. Il ritorno alla finestra Hub dal vivo può causare l’acquisizione e la visualizzazione ripetute dell’interfaccia dell’Hub dal vivo da parte dei contenuti condivisi, con conseguente comportamento visivo imprevisto. Questa è una limitazione nota della condivisione a schermo intero ed è in fase di valutazione per una versione futura.

Condividete un&#39;applicazione o una finestra specifica invece dell&#39;intero schermo. Condividere, ad esempio, la presentazione o la finestra del documento che si desidera visualizzare. La condivisione di una singola applicazione mantiene isolati i contenuti condivisi e impedisce l&#39;effetto di tunneling.

### Il report del dashboard della sessione non sarà disponibile immediatamente dopo il rientro a una sessione terminata

Il report Dashboard della sessione non sarà disponibile immediatamente dopo il rientro a una sessione terminata
Se un Istruttore visualizza il dashboard di sessione dopo che una sessione è terminata e quindi si ricongiunge alla stessa sessione utilizzando il relativo URL, il report Dashboard di sessione esistente potrebbe non essere più disponibile. L’aggiornamento del report può richiedere da cinque a sette minuti circa e il report può essere nuovamente disponibile durante l’elaborazione dell’istanza della sessione precedente. Questa è una limitazione nota ed è in fase di valutazione per una versione futura.

Attendi da cinque a sette minuti dopo aver chiuso la sessione prima di riaprire l’URL del dashboard. Ciò consente all&#39;istanza della sessione precedente di completare l&#39;elaborazione, dopodiché il report Dashboard della sessione diventa nuovamente disponibile.
...
La versione di aprile 2026 di Adobe Learning Manager introduce miglioramenti per l’esperienza degli Allievi, l’amministrazione, la creazione di contenuti e la creazione di report, con diverse funzioni attualmente in versione beta e soggette a modifiche.

**Miglioramenti dell’esperienza di apprendimento**

* Navigazione del lettore Fluidic migliorata grazie all’etichettatura &quot;Next Module&quot; chiara e a un’azione di uscita dedicata per un migliore flusso del corso.
* Supporto multilingue esteso a risorse formative, elenchi di controllo e didascalie video.
* Assistente AI integrato nell&#39;esperienza di apprendimento per fornire assistenza contestuale.
* Esperienza di Captivate del contenuto migliorata con sommari, indicatori di completamento e funzioni di esportazione delle note unificate.
* Supporto per l’impostazione della lingua del lettore tramite LTI per un’esperienza uniforme su più piattaforme.

**Aggiornamenti per amministratori e autori**

* Zoom Connector ora supporta più sessioni virtuali simultanee, migliorando l&#39;efficienza della pianificazione.
* Possibilità di limitare i tempi di avvio dei moduli e di migliorare la visibilità della scadenza degli oggetti di apprendimento tramite le API.
* Moduli elenco di controllo migliorati con le opzioni di punteggio ponderato, supporto multilingue e feedback dei revisori.
* Certificati personalizzati aggiornati con un editor drag-and-drop, campi dinamici e sfondi generati dall’intelligenza artificiale.
* Introduzione di Experience Builder senza accesso per creare pagine di apprendimento pubbliche.
* I corsi condivisi ora visualizzano correttamente l’autore originale.

**Funzionalità Istruttore**

* Generazione di codici QR per l’iscrizione alla sessione e il tracciamento della partecipazione.
* Possibilità di fornire commenti e feedback durante le valutazioni dell’elenco di controllo.

**Miglioramenti a livello di reporting e analisi**

* Report SCORM migliorati per acquisire più tentativi di quiz.
* Calcolo più accurato del tempo di apprendimento nelle trascrizioni degli allievi.
* Aggiornamenti ai report di trascrizione per gli Amministratori.
* Funzionalità di ricerca avanzata migliorata.

**Equivalenti e alternative**

* I miglioramenti apportati agli equivalenti e alle alternative migliorano la flessibilità dei percorsi di apprendimento consentendo una sostituzione e una mappatura più accurate dei corsi.

**Impatto chiave**
Questi aggiornamenti migliorano usabilità, scalabilità e automazione, con particolare attenzione all&#39;apprendimento basato sull&#39;intelligenza artificiale, al supporto multilingue, alla gestione semplificata della formazione virtuale e alla creazione di report più accurati.

Per ulteriori informazioni, vedere [Novità e modifiche nella versione di aprile 2026 di Adobe Learning Manager](/help/migrated/whats-new.md).

## Bug corretti in questa release

**Ricerca avanzata:** la ricerca avanzata non rispetta l&#39;accesso agli oggetti di apprendimento in base all&#39;iscrizione dell&#39;utente quando manca l&#39;accesso al catalogo. La ricerca avanzata ora applica gli stessi controlli di accesso basati sull’iscrizione della ricerca lessicale, consentendo agli utenti iscritti di accedere all’LO senza l’accesso al catalogo.

**Adobe Experience Manager:** L’intestazione del widget del catalogo non conserva il catalogo selezionato e apre la pagina del catalogo con tutti gli oggetti di apprendimento anziché il catalogo filtrato. Quando si fa clic sull’intestazione di un widget del catalogo, viene aperta la pagina del catalogo in cui sono preselezionati i cataloghi corrispondenti e gli oggetti di apprendimento sono stati filtrati correttamente sul catalogo.

**App per dispositivi mobili:** Su dispositivi mobili, gli Allievi potevano completare tutti gli elementi nella prima sezione di un percorso di apprendimento ordinato, ma la seconda sezione non si sbloccava anche dopo il completamento corretto. Una volta completati tutti gli elementi di una sezione su dispositivo mobile, la sezione successiva di un percorso di apprendimento ordinato viene ora sbloccata correttamente per l’Allievo.

**Migrazione LMS:** quando un&#39;istanza di corso creata tramite la migrazione aveva un ID istanza alfanumerico, la riesecuzione della migrazione per aggiornare una sessione VC ha creato una nuova sessione invece di aggiornare quella esistente. La migrazione ora aggiorna correttamente le sessioni VC esistenti per le istanze del corso con ID alfanumerici, anziché creare sessioni duplicate.

**Durata del corso:** Per i corsi VILT, la durata a livello di corso veniva sempre ricavata da un LO_DURATION statico nell’istanza predefinita, diventato 0 quando le sessioni venivano ritirate e non veniva ricalcolata dai moduli istanza effettivi. La durata del corso VILT ora riflette i moduli di istanza effettivi anziché basarsi solo su un valore LO_DURATION non aggiornato o pari a zero dall’istanza predefinita.

**Connettori:** poiché il controller embber module-details è stato riutilizzato tra i moduli, vcHostingSystem non è stato aggiornato in modo coerente, causando un comportamento misto VC/F2F e cancellando in modo errato instructorId basato solo su vcHostingSystem. La gestione dei moduli ora mantiene vcHostingSystem coerente e cancella instructorId solo per le sessioni VC vere, quindi gli istruttori vengono aggiornati correttamente per le sessioni F2F convertite.

**Migrazione LMS:** la colonna CSV di LearningProgramCourse per l’ordine del corso non è stata rispettata dal backend, pertanto l’ordine dei corsi nelle sezioni LP è stato ignorato anche quando orderEnforced era true. La colonna CSV non utilizzata per l’ordine del corso è stata rimossa per abbinare il comportamento back-end ed evitare che l’ordine tramite CSV non sia supportato.

**Prestazioni:** un flusso di lavoro di automazione AWS ha inviato ripetutamente richieste di download con codifica errata (con uno spazio non codificato) all&#39;API HTTP Akamai CMS, causando frammentazioni di 500 errori e un traffico eccessivo non valido. Il flusso di lavoro automatizzato ora codifica correttamente gli spazi nell’URL di download, impedendo richieste non valide ed eliminando i 500 picchi di errore associati.

**Iscrizione e annullamento dell&#39;iscrizione:** In alcuni flussi di iscrizione come Amministratore per programmi di apprendimento, certificazioni e corsi, l&#39;origine dell&#39;iscrizione è stata registrata in modo errato come SELF_ENROLL anziché come ADMIN_ENROLL. Le iscrizioni guidate dall’amministratore sono ora archiviate in modo coerente con l’origine ADMIN_ENROLL nei flussi interessati.

**Iscrizione e annullamento dell’iscrizione:** per le iscrizioni basate su soglie in una catena di certificazioni di ricorrenza, gli allievi sono stati iscritti a una ricorrenza successiva anche quando non avevano completato la ricorrenza precedente. L’iscrizione basata su soglie ora rispetta la catena di ricorrenza e gli allievi non vengono iscritti a una certificazione successiva a meno che non abbiano completato la ricorrenza precedente richiesta.

**Impostazioni amministratore:** gli amministratori non possono aggiungere un account condiviso tra pari il cui sottodominio è iniziato con un numero, a causa di una regola di convalida che richiede che il sottodominio inizi con una lettera. Gli account condivisi tra pari con sottodomini che iniziano con un numero sono ora accettati, a condizione che soddisfino le regole di carattere consentite.

**Corso - Punteggio quiz:** All&#39;avvio headless, dopo che un amministratore ha reimpostato un modulo quiz, il lettore ha mostrato un totale errato di 12 domande invece delle 3 effettive. All’avvio headless, il lettore del quiz ora visualizza il numero totale corretto di domande dopo il ripristino del modulo.

**Autore:** Quando un gruppo di contenuti è stato aggiornato da SCORM a CAPI, le informazioni di reporting e i criteri di completamento non sono stati aggiornati, lasciando il gruppo di contenuti senza criteri di completamento. L&#39;aggiornamento di un gruppo di contenuti da SCORM a CAPI ora aggiorna correttamente le informazioni di reporting e imposta i criteri di completamento appropriati per il gruppo di contenuti.

**App per dispositivi mobili:** Nell&#39;app per dispositivi mobili Captivate Prime, le risorse scaricabili aperte in una nuova scheda tramite target=&quot;_blank&quot; non sono state scaricate quando sono state toccate. Nell’app per dispositivi mobili, toccando una risorsa scaricabile ora il download viene avviato correttamente anche quando il contenuto utilizza i collegamenti target=&quot;_blank&quot;.

**Editor di testo RTF:** Le interruzioni di riga nelle descrizioni degli oggetti di apprendimento di testo RTF sono state rimosse nella visualizzazione Autore dopo la pubblicazione, anche se gli allievi visualizzavano ancora le interruzioni di riga. Le descrizioni degli oggetti di apprendimento in formato testo RTF ora mantengono e visualizzano correttamente le interruzioni di riga sia nella visualizzazione Autore che in quella per allievi.

## Problema noto/Per progettazione

### Comportamento del controllo delle versioni per le risorse di Second Nature

Adobe Learning Manager (ALM) non incrementa la versione delle risorse di Second Nature (SN) durante gli scenari di modifica. Questo comportamento riflette il modo in cui ALM gestisce gli aggiornamenti del contenuto SN anziché una scelta di creazione.
...
+++Aggiornamento 104: Assistente all’intelligenza artificiale per gli Allievi in Adobe Learning Manager

Data di pubblicazione: 2 marzo 2026

## Assistente AI per gli Allievi

Uno strumento di conversazione basato su GenAI progettato per aiutare gli Allievi a trovare immediatamente risposte, risorse e corsi pertinenti utilizzando query sul linguaggio naturale.

**Punti salienti**

* Utilizza Retrieval-Augmented Generation (RAG) per cercare in modo intelligente contenuti di corsi, risorse formative e materiali didattici per fornire risposte accurate e contestuali.
* Comprende il contesto, conserva le interazioni precedenti e fornisce risposte basate sulle citazioni come fonte di verità, che possono essere utilizzate in un secondo momento in base alle esigenze.
* Consente agli Allievi di richiedere script personalizzati, punti di discussione e riepiloghi personalizzati in base alle proprie esigenze di apprendimento.
* Esegue la ricerca in tutti i formati di contenuto supportati, inclusi PDF, DOCX, PPTX, Audio, Video e SCORM 1.2 &amp; 2004.

Per ulteriori informazioni, visualizza [Assistente IA per gli Allievi](/help/migrated/learners/feature-summary/learner-ai-assistant.md).
...
Experience Builder è ora disponibile per tutti gli account Adobe Learning Manager, consentendo ai clienti di creare esperienze di apprendimento con marchio completo utilizzando componenti a basso codice e layout flessibili. Experience Builder consente agli amministratori di creare portali di apprendimento personalizzati e pagine che rispondono perfettamente ai dispositivi mobili.

Per gli account esistenti, l&#39;aggiornamento di Experience Builder verrà implementato automaticamente. La funzione Experience Builder non è disponibile per gli account che continuano a utilizzare la precedente interfaccia utente o l’interfaccia classica.

Per ulteriori informazioni, visualizza [Experience Builder](/help/migrated/administrators/feature-summary/experience-builder/overview.md).

### Interfaccia Web per dispositivi mobili Adobe Learning Manager

L’interfaccia Web mobile è stata ricostruita su un codebase moderno per offrire tempi di caricamento più veloci, tempi di risposta migliori e un’esperienza di apprendimento più intuitiva su tutti i dispositivi.

L’esperienza Web per dispositivi mobili aggiornata è abilitata per tutti i nuovi account per impostazione predefinita. Anche gli account esistenti riceveranno automaticamente l’aggiornamento, tranne quando Apprendimento sociale è abilitato. In questi casi, l’esperienza mobile precedente rimarrà attiva fino a quando l’Apprendimento sociale non verrà migrato al nuovo framework compatibile.

>[!NOTE]
>
>Apprendimento sociale non è supportato nella nuova interfaccia Web per dispositivi mobili di Adobe Learning Manager.

### Miglioramenti del lettore Fluidic

In precedenza, il pulsante Avanti nel lettore Fluidic non mostrava il titolo del modulo successivo, rendendo difficile per gli Allievi anticipare il passaggio successivo nel percorso di apprendimento.

Con questo miglioramento, il pulsante Avanti ora include il titolo del modulo successivo, che consente agli Allievi di:

* Prima di continuare, individua i contenuti disponibili.
* Naviga tra i moduli con maggiore chiarezza e sicurezza.
* Mantenere un contesto migliore durante l’intero flusso del corso.

![](assets/fluidic-player-next.png)
_Il lettore Fluidic ora mostra il pulsante Avanti con il titolo del modulo successivo_

### Modulo di feedback L1

Il feedback L1 poteva essere configurato solo all’interno dei singoli oggetti di apprendimento, il che limitava la flessibilità e impediva agli amministratori di utilizzare moduli diversi nel catalogo. Gli Amministratori possono ora creare e utilizzare più moduli di feedback L1 e ogni modulo di feedback può essere assegnato a corsi, percorsi di apprendimento e certificazioni specifici. Il modulo ora supporta diversi tipi di domande, tra cui Efficacia del corso, Server dei criteri di rete, Scala di collegamento e Testo libero.

I moduli di feedback possono essere tradotti in altre lingue, mantenendo la stessa struttura delle domande in tutte le lingue. I report ora includono Nome modulo feedback e Versione modulo feedback per identificare il modulo acquisito da ogni risposta.

**Casi di utilizzo**

* Assegna moduli di feedback personalizzati a diversi programmi di formazione. Ad esempio, i corsi sulle vendite possono utilizzare domande incentrate su Server dei criteri di rete, mentre i corsi sulla conformità possono utilizzare domande su scala Likert ed Efficacia del corso per misurare la chiarezza e la comprensione delle politiche.
* Usa moduli di feedback separati in base ai moduli di apprendimento. Ad esempio, tre diversi moduli per moduli combinati, moduli a ritmo personalizzato e moduli classe.

Per ulteriori informazioni, visualizzare il [modulo di feedback L1](/help/migrated/administrators/feature-summary/l1-feedback-form.md).

+++

+++Aggiornamento 102: versione di ottobre 2025 di Adobe Learning Manager

**Data di pubblicazione**: 12 ottobre 2025

## Novità di questa versione

Per ulteriori informazioni, consultate [Novità della versione di ottobre 2025 di Adobe Learning Manager](/help/migrated/whats-new.md).
+++

+++Aggiornamento 101: versione di luglio 2025 di Adobe Learning Manager
...
## Controllo versione di contenuti migliorato per gli Allievi che hanno completato un corso
...
|---|---|
| Applicare l’aggiornamento del contenuto a tutti gli Allievi | Aggiorna tutti gli Allievi ora:Apply aggiornamento contenuti per tutti gli Allievi |
...
Per ulteriori informazioni, fai riferimento alla pagina [Novità di Adobe Learning Manager](/help/migrated/whats-new-may-2025.md).
+++
...
Ogni volta che tentavi di recuperare le informazioni sulla versione del modulo utilizzando AP/corsi/{coursesid}, si verificava un errore. Questo problema è stato risolto.
...
Learning Manager fornisce una funzionalità per generare trascrizioni per gli allievi Learning Manager dell’organizzazione. Per ulteriori informazioni, consulta il [contenuto della guida della funzionalità Trascrizioni allievi](/help/migrated/administrators/feature-summary/reports/learner-transcripts.md).
...
Nel login per gli Allievi, la descrizione comando del nome dell’abilità in **Mappa abilità** non mostrava il nome e cognome. Questo problema è stato risolto.
