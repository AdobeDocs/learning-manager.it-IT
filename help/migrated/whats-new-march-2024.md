...
>[!NOTE]
>
>Guarda questo [webinar](https://learningmanager.adobe.com/app/learner?accountId=98632#/course/9212121) per ulteriori informazioni sulle nuove funzioni di questa versione.
...
Per ulteriori informazioni, visualizza [Più elementi di branding personalizzati](administrators/feature-summary/themes.md#multiple-branding).
...
Se un numero elevato di Allievi è iscritto al corso, ad esempio, più di 50.000, l’elenco degli Allievi non verrà visualizzato. Puoi cercare un Allievo nella barra di ricerca *Cerca Allievi* oppure selezionare il collegamento **Scarica** nella parte superiore della barra di ricerca per scaricare l’elenco degli Allievi.

### Pagina Amministratore: Allievi

Durante la ricerca di qualsiasi utente, le opzioni **Scarica Allievo** e **Esporta** scaricano lo stesso report. Nel frattempo, durante la ricerca di un gruppo di utenti, ora puoi scaricare gli utenti filtrati da quel gruppo di utenti. Durante la ricerca in un gruppo di utenti,
**Scarica elenco allievi** diventa **Scarica elenco allievi per gruppo utenti** L’opzione **Esporta** scarica nuovamente l’intero elenco.
...
Per gli utenti esterni, si applica lo stesso comportamento. Se il numero di utenti è elevato, puoi scaricare gli utenti e recuperare anche i dettagli di un utente dopo una ricerca nel formato *Nome, e-mail | UUID*.

#### Pagina Pulizia utente

Nella pagina Pulizia utente, per gli utenti eliminati, è stata rimossa la funzionalità di ordinamento in data **Data eliminazione**. Puoi ordinare solo in base agli UUID.
...
Nelle versioni correnti di Adobe Learning Manager, se è presente un numero elevato di utenti iscritti a una certificazione, non puoi visualizzare gli Allievi annullati poiché il menu a discesa **Stato** è disabilitato.

In questa versione di Adobe Learning Manager, se il numero di utenti iscritti è elevato, nel menu a discesa **Stato** vengono visualizzate solo due opzioni: **Iscritto** e **Annullato**. L&#39;opzione **Iscritto** è selezionata per impostazione predefinita. Se selezioni **Annulla iscrizione**, viene visualizzato l’elenco degli Allievi non iscritti.

#### Modifiche al gruppo di utenti

Nel caso di un gruppo di utenti, se il numero di utenti nel gruppo è inferiore, ad esempio, a 50.000, il menu a discesa **Stato** visualizza tutte le opzioni: Certificato, Assegnato e In scadenza.

Se il numero di utenti in un gruppo di utenti è elevato, nel menu a discesa **Stato** vengono visualizzate solo due opzioni: **Iscritto** e **Non iscritto**, in base al nuovo progetto.
...
Durante la ricerca di qualsiasi utente, le opzioni **Scarica Allievo** e **Esporta** scaricano lo stesso report. Nel frattempo, durante la ricerca di un gruppo di utenti, ora puoi scaricare gli utenti filtrati da quel gruppo di utenti. Durante la ricerca in un gruppo di utenti, l’opzione **Scarica elenco allievi** diventa **Scarica elenco allievi per gruppo di utenti** L’opzione **Esporta** scarica nuovamente l’intero elenco.
...
In base alle [impostazioni dell&#39;amministratore](administrators/feature-summary/classroom.md#classroom-settings), in qualità di autore puoi [creare, modificare ed eliminare percorsi](administrators/feature-summary/classroom.md#add-classroom-location).
...
In qualità di Amministratore, puoi applicare restrizioni a un Autore per modificare o eliminare un’aula. Per ulteriori informazioni, consulta [Impostazioni classe](administrators/feature-summary/classroom.md#classroom-settings).
...
Per ulteriori informazioni, consulta [Piani di apprendimento](administrators/feature-summary/learning-plans.md).
...
Per ulteriori informazioni, vedere [Stato invio](instructors/feature-summary/learners.md#filter-file-submissions).
...
Durante la creazione di un elenco di controllo, un Autore può selezionare **Abilita** nella sezione Elenco di controllo obbligatorio. In questo modo si impedisce agli Allievi di continuare nel modulo se non rientrano nell’elenco di controllo. Possono procedere solo se superano l&#39;elenco di controllo.
...
Ciò significa che la casella di controllo **Abilita** non è stata selezionata nella sezione Rivalutazione durante la creazione dell&#39;elenco di controllo. Se questa casella di controllo è selezionata, nella pagina Elenco di controllo Istruttore sarà disponibile il pulsante Visualizza/Rivaluta.
...
Per ulteriori informazioni, visualizzare [Creare un elenco di controllo](authors/feature-summary/courses.md#checklist-fail).
...
Nelle versioni correnti di Adobe Learning Manager, un Allievo è contrassegnato come **Non partecipato** quando è iscritto a una sessione Connect VC, ma non soddisfa i criteri di completamento.

In questa versione, lo stato diventa **Non ancora contrassegnato**.
...
Per ulteriori informazioni, consulta l&#39;etichettatura bianca nell&#39;[app mobile Adobe Learning Manager](white-label.md).
...
Se esegui una migrazione per aggiornare un corso, un piano di apprendimento o una certificazione esistenti, il corso, il piano di apprendimento o la certificazione con **codice di accesso univoco** vengono aggiunti all’app Autore.

Durante la migrazione, è necessario aggiornare i valori **uniqueLOId** nei CSV per il corso o il piano di apprendimento o la certificazione anche se si tratta di una colonna facoltativa.

Se la colonna **uniqueLoId** non viene aggiunta prima di eseguire la migrazione durante l’aggiornamento del corso o del piano di apprendimento o della certificazione esistente con **uniqueLOId**, dopo la migrazione i valori **uniqueLOId** verranno sostituiti con valori NULL.

>[!NOTE]
>
>La colonna **uniqueLoId** non è applicabile al file CSV della risorsa formativa.
...
Scarica i file CSV dal [manuale di migrazione](integration-admin/feature-summary/migration-manual.md#csv-specifications-and-sample-csvs).
...
Per ulteriori informazioni sulla configurazione dei connettori, visualizzare [Connettori in Adobe Learning Manager](integration-admin/feature-summary/connectors.md).

### Modifiche al report Accesso

Il report Accesso sarà disponibile solo per gli ultimi cinque trimestri. Se un Amministratore dell’integrazione richiede il download su richiesta dell’esportazione unificata con **accesso di accesso** controllato, Adobe Learning Manager visualizzerà un messaggio di errore. Tuttavia, non vi è alcun impatto sulle altre relazioni.
...
Se provi a scaricare il report **Accesso** che va oltre i cinque trimestri, viene visualizzato un messaggio di errore. Un messaggio di errore simile viene visualizzato se si tenta di scaricare il report **Controllo utente** che va oltre i sei mesi.

### API obsolete

Visualizza [API obsolete in Adobe Learning Manager](api-deprecations-list.md) per un elenco cumulativo di tutte le API obsolete nel prodotto.
...
* Il pulsante **Annulla iscrizione** non viene visualizzato come previsto dopo la prima iscrizione, ma viene visualizzato dopo un aggiornamento.
...
* Nell’app per dispositivi mobili o in modalità immersiva, dopo aver eliminato o ritirato un’istanza del corso e aver selezionato la prossima istanza attiva, viene visualizzato il pulsante **Registra interesse** anziché **Iscriviti**.
...
Visualizza [requisiti di sistema di Adobe Learning Manager](system-requirements.md).
